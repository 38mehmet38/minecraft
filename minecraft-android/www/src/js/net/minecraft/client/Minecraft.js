...
export default class Minecraft {
    ...
    loadWorld(world) {
        if (world === null) {
            ...
            this.loadingScreen.setTitle("Harita olusturuluyor...");
    ...
    onRender(partialTicks) {
        if (this.isInGame()) {
            ...
            // Update touch controller if mobile
            this.window.touchController.update(this.window);
    ...
    onMouseClicked(button) {
        if (this.window.isLocked()) {
            ...
            // Place block / Interact with block
            if (button === 2) {
                if (hitResult != null) {
                    // First check if we can interact with the block
                    let interactBlockId = this.world.getBlockAt(hitResult.x, hitResult.y, hitResult.z);
                    let interactBlock = Block.getById(interactBlockId);

                    if (interactBlock && interactBlock.canInteract()) {
                        // Try to interact with block (chest, door, etc.)
                        let handled = interactBlock.onBlockClicked(
                            this.world,
                            hitResult.x,
                            hitResult.y,
                            hitResult.z,
